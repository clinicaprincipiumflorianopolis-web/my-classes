...
function scheduleOverflow(student, year, monthIndex0) {
  ...
  const extraDays = schedule.filter(
    (entry) => countWeekdayInMonth(year, monthIndex0, Number(entry.day)) > 4
  );
  if (extraDays.length === 0) return null;
  return {
    extraWeekdays: extraDays.map((e) => WEEKDAYS[Number(e.day)])
  };
}
const MEI_LIMIT = 8e4;
...
function Dashboard({
  ...
  }))), overflowEntries.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "pa-card", style: { marginTop: 20, padding: 16, background: "#F3EEFF", border: "1px solid #DCCCFF" } }, /* @__PURE__ */ React.createElement("strong", { style: { color: "#5B3EBF" } }, overflowEntries.length, " aluno", overflowEntries.length > 1 ? "s" : "", " com dia de aula extra esse m\xEAs"), /* @__PURE__ */ React.createElement("p", { className: "pa-muted", style: { margin: "4px 0 0" } }, "Esse m\xEAs tem 5 ocorr\xEAncias de algum dia da semana em vez de 4 \u2014 vale avisar."), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 } }, overflowEntries.map(([sid, info]) => {
    const s = data.students.find((st) => st.id === sid);
    if (!s) return null;
    return /* @__PURE__ */ React.createElement("button", { key: sid, className: "pa-btn secondary", onClick: () => onOpenStudent(sid) }, s.name, " \xB7 ", info.extraWeekdays.join(", "));
